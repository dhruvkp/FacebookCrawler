...
def downloadVideosLiked(driver,userid):
    url = 'https://www.facebook.com/search/'+str(userid).strip()+'/videos-liked'
    driver.get(url)
    if "Sorry, this page isn't available" in driver.page_source:
        print "[!] Cannot access page "+url
        return ""
        ...
        for z in pageName2:
            # if z['src'].endswith('.jpg'):
                ...
                if m:
                    ...
                    writeToMysqlDB('Feed',[photoid,'Photo',"",photoList[3],photoList[1],photoowner,getEntityIdOfUser(photoowner),photoList[0]])
                    writeToMysqlDB('Feeds_likes',[photoid,uid])
    ...
    for x in appsData:
        ...
        if m:
            ...
            for c in comments:
                ...
                timestamp=""
                for z in timestamps[cnt].findChildren("span"):
                    if len(z.findChildren())>0:
                        timestamp=z.findChildren("a")[0].findChildren("abbr")[0]['title']
            ...
            # likeselement = soup1.findAll("div",{"class":"UFILikeSentenceText"})
            # if len(likeselement)>0:
            #     likeselement=likeselement[0]
            #     likeselement=likeselement.contents[0]
            #     if len(likeselement.findChildren("a",{"rel":"dialog"}))>0:
            #         url="https://www.facebook.com"+likeselement.findChildren("a",{"rel":"dialog"})[0]['href']
            #         driver.get(url)
            #         html2=driver.page_source
            #         soup2 = BeautifulSoup(html2)
            #     else:
            #         likes=likeselement.findChildren("a")
            #         for like in likes:
            #             href=like['href']
            #             username2=href[href.rfind("/")+1:len(href)]
            #             uid2=convertUser2ID2(driver,username2)


    return tempList

def parseVideosLiked(html):
    soup = BeautifulSoup(html)
    appsData = soup.findAll("div", {"class" : "_42bw"})
    tempList = []
    for x in appsData:
        ...
        if m:
            ...
            writeToMysqlDB("Feeds_likes",[vid,uid])
            # likeselement = soup1.findAll("div",{"class":"UFILikeSentenceText"})
            # if len(likeselement)>0:
            #     likeselement=likeselement[0]
            #     likeselement=likeselement.contents[0]
            ...
            if m:
                try:
                    ...
                    if not re.search('\d+', value):
                        ...
                        print "friend's user id "+fbID
                        uid2=convertUser2ID2(driver,fbID)
                        writeToMysqlDB("Friends",[uid,fbID])
                        parseUserInfo(fbID,False)
                        ...
                        print "friend's user id "+fbID
                        uid2=convertUser2ID2(driver,fbID)
                        writeToMysqlDB("Friends",[uid,uid2])
                        parseUserInfo(fbID,False)
                        print year
                        print monthDigit
                        friendList.append([str(uid),friendName,fbID,int(monthDigit),int(year)])
...
#    #else:
    ...
    html = downloadVideosLiked(driver,uid)
    dataList = parseVideosLiked(html)
    for x in dataList:
        writeToMysqlDB('Feed',[x[1],"Video",x[2],x[4],x[5],str(uid),entity_id])
